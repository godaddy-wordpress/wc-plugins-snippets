 ...
 */
function sv_wc_csv_export_update_exported_order_status( $mark_as_exported, $order, $export_method, $handler ) {
	...
	$order->update_status( 'completed', 'Order exported to CSV and status completed.' );
}
add_action( 'wc_customer_order_export_csv_order_exported', 'sv_wc_csv_export_update_exported_order_status', 10, 4 );
 ...
 * @param \WC_Order $order order being exported
 */
function sv_wc_csv_export_update_exported_order_status_on_paid( $order ) {

	if ( $order->is_paid() ) {
		$order->update_status( 'completed', 'Order exported to CSV and set to completed.' );
	}
}
add_action( 'wc_customer_order_export_csv_order_exported', 'sv_wc_csv_export_update_exported_order_status_on_paid', 10, 2 );
