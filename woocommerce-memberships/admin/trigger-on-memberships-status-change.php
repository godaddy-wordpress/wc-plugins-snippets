 ...
 * @param string $new_status the new membership status
 */
function sv_wc_memberships_reactivate_member_role( $user_membership, $old_status, $new_status ) {
	...
	if ( in_array( 'site_member', $roles ) || ! in_array( 'customer', $roles ) || ! wc_memberships_is_user_active_member( $user_id, $user_membership->get_plan_id() ) ) {
		return;
	}
...
add_action( 'wc_memberships_user_membership_status_changed', 'sv_wc_memberships_reactivate_member_role', 10, 3 );
